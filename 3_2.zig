...
pub fn main() !void {
    ...
    var grid = try allocator.alloc(u32, h * w);
    defer allocator.free(grid);
    std.mem.set(u32, grid, 0);

    // Trace wire1 path onto grid with steps at each point
    {
        ...
        for (wire1_path[0..]) |p| {
            ...
            while (i < p.len) : ({
                ...
                if (x != origin_x and y != origin_y) {
                    grid[y + w * x] |= (steps << 2) | @enumToInt(Marker.Wire1);
                }
    ...
    // Trace wire2 path onto grid and compute minimum steps per intersection
    {
        var x: usize = origin_x;
        var y: usize = origin_y;
        var steps: u30 = 0;
        ...
        for (wire2_path[0..]) |p| {
            ...
            while (i < p.len) : ({
                ...
                if (grid[y + w * x] & 0b11 == @enumToInt(Marker.Both)) {
                    var wire1_steps = grid[y + w * x] >> 2;

                    if (steps + wire1_steps < min_steps) {
                        min_steps = steps + wire1_steps;
                    }
                }
