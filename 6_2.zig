...
// `to` must be a direct/indirect orbiter of `from`.
fn countPath(from: *Node, to: *Node, steps: usize) ?usize {
    if (std.mem.eql(u8, from.id, to.id)) {
        return steps - 1; // Discard the hop from the initial orbit
    }

    for (from.orbiters.toSliceConst()) |orbiter| {
        if (countPath(orbiter, to, steps + 1)) |found| {
            return found;
        }
    }
...
pub fn main() !void {
    ...
    var you = space.getValue("YOU").?;
    var san = space.getValue("SAN").?;

    var path = std.StringHashMap(void).init(allocator);
    defer path.deinit();

    // Traverse from each to COM, then determine where the first common node is and compute
    // the difference. parent is never NULL since the graph is assumed to be well formed,
    // terminating at COM.
    var n = you;
    while (!std.mem.eql(u8, n.parent.?.id, "COM")) {
        try path.putNoClobber(n.id, {});
        n = n.parent.?;
    }

    n = san;
    while (!path.contains(n.id)) {
        n = n.parent.?;
    }

    var a = countPath(n, you, 0) orelse 0;
    var b = countPath(n, san, 0) orelse 0;
